...
; (function ($w) {
  ...
  jm.node = function (sId, iIndex, sTopic, oData, bIsRoot, oParent, eDirection, bExpanded,diseditable) {
  ...
  jm.mind.prototype = {
    ...
    set_root: function (nodeid, topic, data,diseditable) {
      if (this.root == null) {
        this.root = new jm.node(nodeid, 0, topic, data, true,undefined,undefined,undefined,diseditable)
        console.log('27-',this.root);
    ...
    add_node: function (parent_node, nodeid, topic, data, idx, direction, expanded,diseditable) {
      ...
      if (!jm.util.is_node(parent_node)) {
        ...
        if (!the_parent_node) {
          ...
          return this.add_node(the_parent_node, nodeid, topic, data, idx, direction, expanded,diseditable)
        }
      }
      var nodeindex = idx || -1
      var node = null
      if (parent_node.isroot) {
        ...
        node = new jm.node(nodeid, nodeindex, topic, data, false, parent_node, d, expanded,diseditable)
      } else {
        node = new jm.node(nodeid, nodeindex, topic, data, false, parent_node, parent_node.direction, expanded,diseditable)
      }
  ...
  jm.format = {
    node_tree: {
      ...
      _parse: function (mind, node_root) {
        console.log('14-',node_root);
        var df = jm.format.node_tree
        var data = df._extract_data(node_root)
        mind.set_root(node_root.id, node_root.topic, data,node_root.diseditable)
      ...
      _extract_subnode: function (mind, node_parent, node_json) {
        ...
        var node = mind.add_node(node_parent, node_json.id, node_json.topic, data, null, d, node_json.expanded,node_json.diseditable)
        console.log('15-',node);
      ...
      _extract_subnode: function (mind, parentid, node_array) {
        ...
        while (i--) {
          ...
          if (node_json.parentid == parentid) {
            ...
            mind.add_node(parentid, node_json.id, node_json.topic, data, null, d, node_json.expanded,node_json.diseditable)
            console.log('7-');
            node_array.splice(i, 1)
            extract_count++
  ...
  jm.prototype = {
    ...
    begin_edit: function (node) {
      ...
      console.log('5-',node);
      if (this.get_editable() && !node.diseditable) {
  ...
  jm.view_provider.prototype = {
    ...
    // TODO:初始化node节点
    init_nodes: function () {
      console.log('12-',this.jm.mind.nodes);
      var nodes = this.jm.mind.nodes
    ...
    edit_node_begin: function (node) {
      ...
      element.style.color = node.data['foreground-color']
      this.e_editor.focus()
    ...
    _reset_node_custom_style: function (node_element, node_data) {
      ...
      if ('foreground-color' in node_data) {
        console.log('30-',node_element.innerHTML);
        if(node_element.innerHTML && node_element.innerHTML == '请输入内容'){
          node_element.style.color = '#999'
          console.log('36-');
        }else{
